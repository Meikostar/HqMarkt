...
public abstract class RxAsyncTask<Param, Progress, Result> {
    ...
    private final void rxTask(final Param... params) {
        ...
        Flowable.create((FlowableOnSubscribe<Result>) e -> {
    ...
    protected void publishProgress(final Progress... progresses) {
        if (mFlowable2 == null) {
            mFlowable2 = Flowable.create((FlowableOnSubscribe<Progress[]>) e -> e.onNext(progresses), BackpressureStrategy.BUFFER).observeOn(AndroidSchedulers.mainThread());
        }
        mFlowable2.subscribe(progress -> onProgressUpdate(progresses));
    }
